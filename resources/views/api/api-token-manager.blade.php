<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('Créer un jeton d\'API') }}
        </x-slot>

        <x-slot name="description">
            {{ __('Les jetons API permettent aux services tiers de s\'authentifier auprès de notre application en votre nom.') }}
        </x-slot>
        ...
        <x-slot name="form">
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('Autorisations') }}" />
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Veuillez copier votre nouveau jeton d\'API. Pour votre sécurité, il ne sera plus affiché.') }}
            </div>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        ...
        <x-slot name="content">
            {{ __('Voulez-vous vraiment supprimer ce jeton d\'API ?') }}
        </x-slot>
